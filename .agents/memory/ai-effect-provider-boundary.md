...
**Why:** A live provider returned an `effectConfig` wrapper around an otherwise valid effect. Silently unwrapping model output would weaken the data-only boundary and could allow unsupported payloads into admin forms.

**How to apply:** Keep provider credentials server-side, require an explicit JSON envelope, validate against the current shared DSL, and make the UI apply only local form state; the existing Admin save remains the only persistence path.