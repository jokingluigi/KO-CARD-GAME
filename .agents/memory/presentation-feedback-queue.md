...
**How to apply:** Consume appended events with a cursor, clear cosmetic queues when the event log resets, preserve event order, use DOM refs or cached geometry for removed targets, and make reduced-motion completion advance by a short timer rather than normal travel duration. Prefer explicit effect events when the engine gains them; until then, infer untagged stat changes only when no damage/removal event explains the delta.