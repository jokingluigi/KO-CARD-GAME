...
import { ArrowLeft, Box, Library, Shield, Sparkles } from "lucide-react";
import { fetchCollection, type Collection } from "@/lib/collection-client";
...
export default function CollectionPage() {
  ...
  useEffect(() => {
    fetchCollection()
      .then((nextCollection) => { setCollection(nextCollection); setMessage(""); })
      .catch((error: Error) => {
    ...
    <main className="min-h-screen bg-neutral-950 px-5 py-8 text-neutral-100 sm:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mb-10">
          ...
          <div className="flex flex-col items-start gap-3 rounded-xl border border-neutral-800 bg-black/40 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-black">내 팩에서 보상 받기</h3><p className="mt-1 text-sm text-neutral-500">서버에서 보상을 확정하고 안전하게 한 장씩 공개합니다.</p></div><button type="button" onClick={() => { window.location.href = `${import.meta.env.BASE_URL.replace(/\/$/, "")}/packs`; }} className="flex items-center gap-2 rounded bg-primary px-4 py-3 text-sm font-black text-black hover:bg-yellow-400"><Library className="h-4 w-4" /> 내 팩 열기</button></div>
        </section>
