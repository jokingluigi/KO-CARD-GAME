...
export type LobbyRoomMember = LobbyDeckSummary & { userId: string; nickname: string; ready: boolean };
export type LobbyRoomState = { roomId: string; roomCode: string; host: LobbyRoomMember; guest: LobbyRoomMember | null; youAreHost: boolean };
...
export type OnlineLobbyMessage =
  ...
  | { type: "LOBBY_ERROR" | "ERROR"; code: string; message: string };
...
export type OnlineMatchMessage =
  | ({ type: "MATCH_SNAPSHOT"; seat: "PLAYER_ONE" | "PLAYER_TWO" } & MatchPayload)
  | ({ type: "ACTION_ACCEPTED"; requestId: string } & MatchPayload)
  | { type: "ACTION_REJECTED"; matchId: string; requestId?: string; code: string; message: string; currentVersion: number }
  | ({ type: "MATCH_ENDED" | "RESYNC_REQUIRED" } & MatchPayload)
  | { type: "MATCH_CONNECTION_STATUS"; matchId: string; playerId: "PLAYER_ONE" | "PLAYER_TWO"; status: "CONNECTED" | "DISCONNECTED_GRACE" | "FORFEITED"; reconnectDeadlineAt: number | null; serverTime: number }
...
export type OnlineLobbyClientMessage =
  ...
  | { type: "SUBSCRIBE" | "UNSUBSCRIBE" | "RESYNC"; matchId: string }
  | { type: "MATCH_ACTION"; matchId: string; requestId: string; expectedVersion: number; action: OnlineActionPayload };
...
class OnlineLobbyClient {
  ...
  onMessage(listener: OnlineLobbyListener) {
    ...
    if (this.lastHandoff) {
      ...
      queueMicrotask(() => { if (this.listeners.has(listener)) listener(handoff); });
    }
    return () => this.listeners.delete(listener);
  }
  onConnectionState(listener: OnlineLobbyConnectionListener) {
    this.connectionListeners.add(listener); listener(this.connectionState);
