...
export function AdminEffectAiGenerator({
  ...
  async function generate() {
    ...
    if (!trimmed) {
      setError("AI로 변환할 자연어 효과를 입력해 주세요.");
      return;
    }
    setBusy(true);
    setError("");
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("message" in body && body.message ? body.message : "AI 효과 초안을 만들지 못했습니다.");
      }
      setDraft(body as EffectAiDraft | EffectAiClarification);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "AI 효과 초안을 만들지 못했습니다.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="md:col-span-2 rounded border border-violet-900/70 bg-violet-950/15 p-3" data-testid="admin-effect-ai-generator">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h4 className="text-sm font-black text-violet-200">AI 효과 생성 도우미</h4>
          <p className="mt-1 text-[11px] leading-relaxed text-neutral-400">
            서버에서 현재 Effect Registry와 CardDefinition을 검증한 초안만 만들며, 적용해도 저장 버튼을 누르기 전에는 DB가 바뀌지 않습니다.
          </p>
        </div>
        <span className="rounded border border-violet-800 px-2 py-1 text-[10px] font-bold text-violet-300">ADMIN ONLY · DRAFT</span>
      </div>
      ...
      {ready && (
        <div className="mt-3 rounded border border-emerald-800 bg-emerald-950/20 p-3" data-testid="ai-effect-draft">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <strong className="text-emerald-200">검증된 AI 효과 초안</strong>
            <span className="text-[10px] font-bold text-emerald-300">서버 validation 통과</span>
          </div>
