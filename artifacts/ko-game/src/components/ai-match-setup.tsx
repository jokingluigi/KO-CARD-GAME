    ...
    <main className="min-h-screen bg-[#080808] px-5 py-10 text-white sm:px-8">
      <section className="mx-auto flex min-h-[calc(100vh-5rem)] w-full max-w-6xl flex-col justify-center">
        ...
        {decks === null || aiDecks === null ? (
          ...
          <div className="mx-auto w-full max-w-2xl rounded-xl border border-amber-900/60 bg-amber-950/20 p-8 text-center"><p className="text-lg font-black text-amber-200">사용할 수 있는 내 덱이 없습니다.</p><p className="mt-2 text-sm leading-6 text-neutral-400">Champion을 포함하고 공개 카드 20~30장으로 구성된 유효한 덱을 먼저 만들어 주세요.</p><button type="button" onClick={onBack} className="mt-6 rounded-lg bg-amber-400 px-5 py-3 text-sm font-black text-black hover:bg-amber-300">메인 메뉴로</button></div>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <section>
                ...
                <div className="grid gap-3">
                  {validDecks.map((deck) => <button key={deck.id} type="button" onClick={() => setSelectedDeckId(deck.id)} className={`rounded-xl border p-4 text-left transition ${selectedDeckId === deck.id ? "border-amber-400 bg-amber-950/30" : "border-neutral-800 bg-neutral-950 hover:border-neutral-600"}`}><div className="flex items-start justify-between gap-4"><div><p className="font-black">{deck.name}</p><p className="mt-1 text-xs font-bold text-neutral-500">{deck.cards.length}장 · {deck.champion?.name ?? "Champion 없음"}</p></div>{selectedDeckId === deck.id && <CheckCircle2 className="h-5 w-5 text-emerald-300" />}</div></button>)}
                </div>
              </section>
              <section>
                ...
                <div className="grid gap-3">
                  ...
                  {validAIDecks.map((deck) => <button key={deck.id} type="button" onClick={() => setSelectedAIDeckId(deck.id)} className={`rounded-xl border p-4 text-left transition ${selectedAIDeckId === deck.id ? "border-amber-400 bg-amber-950/30" : "border-neutral-800 bg-neutral-950 hover:border-neutral-600"}`}><div className="flex items-start justify-between gap-4"><div><p className="font-black">{deck.name}</p><p className="mt-1 text-xs font-bold text-neutral-500">{deck.cards.length}장 · {deck.champion?.name ?? "Champion 없음"}</p><p className="mt-2 line-clamp-2 text-xs leading-5 text-neutral-500">{deck.description || "설명이 없는 AI 덱입니다."}</p></div>{selectedAIDeckId === deck.id && <CheckCircle2 className="h-5 w-5 text-emerald-300" />}</div></button>)}
                </div>
              </section>
            </div>
