...
export function AdminRewardsManager({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  async function saveQuest() {
    setSaving(true);
    const body = { ...quest, targetValue: Number(quest.targetValue), rewardType: "CURRENCY", rewardAmount: Number(quest.rewardAmount), cardType: quest.cardType || null };
    try {
  ...
  async function saveAttendance() {
    setSaving(true);
    const body = { dayIndex: Number(attendanceDay), rewardType: "CURRENCY", rewardAmount: Number(attendanceAmount), enabled: attendanceEnabled };
    try {
    ...
    <section className="space-y-6 rounded-xl border border-neutral-800 bg-black/40 p-5 sm:p-8">
      ...
      <section className="rounded-lg border border-neutral-800 bg-neutral-950/70 p-5">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <input type="number" min="1" value={quest.rewardAmount} onChange={(event) => setQuest({ ...quest, rewardAmount: event.target.value })} placeholder="보상 크레딧" className="rounded border border-neutral-700 bg-black px-3 py-2 text-sm text-white" />
      ...
      <section className="rounded-lg border border-neutral-800 bg-neutral-950/70 p-5">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3"><input type="number" min="1" max="365" value={attendanceDay} onChange={(event) => setAttendanceDay(event.target.value)} placeholder="Day" className="rounded border border-neutral-700 bg-black px-3 py-2 text-sm text-white" /><input type="number" min="1" value={attendanceAmount} onChange={(event) => setAttendanceAmount(event.target.value)} placeholder="보상 크레딧" className="rounded border border-neutral-700 bg-black px-3 py-2 text-sm text-white" /><label className="flex items-center gap-2 text-xs font-bold text-neutral-300"><input type="checkbox" checked={attendanceEnabled} onChange={(event) => setAttendanceEnabled(event.target.checked)} /> 활성화</label></div>
        ...
        <div className="mt-5 grid gap-2 sm:grid-cols-4">{data?.attendance.map((item) => <button key={item.dayIndex} type="button" onClick={() => { setAttendanceDay(String(item.dayIndex)); setAttendanceAmount(String(item.rewardAmount)); setAttendanceEnabled(item.enabled); }} className="rounded border border-neutral-800 px-3 py-3 text-left hover:border-amber-700"><strong className="text-sm text-white">Day {item.dayIndex}</strong><p className="mt-1 text-xs text-amber-200">+{item.rewardAmount} 크레딧</p><p className="mt-1 text-[10px] text-neutral-500">{item.enabled ? "활성" : "비활성"}</p></button>)}</div>
      </section>
    </section>
  );
}