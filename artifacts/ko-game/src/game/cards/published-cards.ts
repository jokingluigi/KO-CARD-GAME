  ...
  if (effectId === "STRUCTURED_EFFECTS_V1" && Array.isArray(config.effects)) {
    ...
    for (const raw of config.effects) {
      ...
      if (!["ENTER_FIELD", "LEAVE_FIELD", "ACTIVE"].includes(trigger as string) ||
           !["BUFF", "DAMAGE", "HEAL", "SILENCE", "DESTROY", "ADD_GOLD", "ADD_NEXT_TURN_GOLD", "DRAW", "REDUCE_COST", "INCREASE_COST", "STUN", "ADD_KEYWORD", "REMOVE_KEYWORD", "SWAP_STATS"].includes(action as string) ||
