...
test('각 선수는 자기 턴에 한 번만 공격할 수 있다', () => {
  ...
  const attacked = attack(initial, 'player-1', 'a', {
    type: 'PLAYER',
    playerId: 'player-2',
  });
...
test('서로 다른 선수는 각각 한 번씩 공격할 수 있다', () => {
  ...
  const first = attack(initial, 'player-1', 'a', {
    type: 'PLAYER',
    playerId: 'player-2',
  });
  const second = attack(first, 'player-1', 'b', {
    type: 'PLAYER',
    playerId: 'player-2',
  });
...
test('다음 자기 턴 시작 시 공격 가능 상태가 된다', () => {
  const initial = combatState([wrestler('a', 1, 2, true)]);
  const playerTwoTurn = endTurn(initial, 'player-1');
  const playerOneTurn = endTurn(playerTwoTurn, 'player-2');
  const state = attack(playerOneTurn, 'player-1', 'a', {
    type: 'PLAYER',
    playerId: 'player-2',
  });
...
test('선수 전투는 서로 동시에 피해를 준다', () => {
  ...
  const state = attack(initial, 'player-1', 'attacker', {
    type: 'WRESTLER',
    playerId: 'player-2',
...
test('선수는 상대 플레이어 본체를 공격할 수 있다', () => {
  ...
  const state = attack(initial, 'player-1', 'a', {
    type: 'PLAYER',
    playerId: 'player-2',
  });
